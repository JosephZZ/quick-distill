...
LOCK_FILE="/tmp/eval_m1.5b_t4b_all_steps_gpu0.lock"
exec 9>"$LOCK_FILE"
if ! flock -n 9; then
  echo "$(date -Is) another eval lane(gpu0) is running, exit." >> "$REPO_DIR/logs/eval_m1.5b_t4b_all_steps_gpu0.log"
  exit 0
fi
...
MASTER_LOG="$REPO_DIR/logs/eval_m1.5b_t4b_all_steps_gpu0.log"
...
need_eval_coding() {
  local outdir="$1"
  [[ ! -f "$outdir"/humaneval_*.jsonl || ! -f "$outdir"/mbpp_*.jsonl ]]
}
...
log "EVAL LANE START gpu0 (t4b)"
for s in "${STEPS[@]}"; do eval_coding_step "scale-m1.5b-t4b-coding-fullseq" "$s"; done
for s in "${STEPS[@]}"; do eval_funcall_step "scale-m1.5b-t4b-funcall-fullseq" "$s"; done
log "EVAL LANE DONE gpu0 (t4b)"
