...
cd "$REPO_DIR" || exit 1
mkdir -p logs
LOCK_FILE="/tmp/m1.5b_t4b_fullseq_queue_gpu0.lock"
exec 9>"$LOCK_FILE"
if ! flock -n 9; then
  echo "$(date -Is) another instance is running, exit." >> "$REPO_DIR/logs/m1.5b_t4b_fullseq_gpu0.queue.log"
  exit 0
fi
...
MASTER_LOG="$REPO_DIR/logs/m1.5b_t4b_fullseq_gpu0.queue.log"
...
_log "QUEUE START t4b GPU0"
run_one "coding" "coseal/CodeUltraFeedback_binarized" "$CODING_SYS" "checkpoints/scale-m1.5b-t4b-coding-fullseq" "scale-m1.5b-t4b-coding-fullseq" "--problem_field instruction"
run_one "funcall" "data/funcall/train.jsonl" "$FUNCALL_SYS" "checkpoints/scale-m1.5b-t4b-funcall-fullseq" "scale-m1.5b-t4b-funcall-fullseq" "--problem_field problem"
_log "QUEUE DONE t4b GPU0"
