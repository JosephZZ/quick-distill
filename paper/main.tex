...
\textbf{Problem:} Convert $(0,3)$ from rectangular to polar coordinates. \\[4pt]
\colorbox{blue!10}{\parbox{0.93\textwidth}{\textbf{First 100 tokens (strategy):} ``Let's solve the problem step-by-step. \textbf{1. Understand the Conversion Formulae:} The radius $r$ in polar coordinates is given by $r = \sqrt{x^2+y^2}$. The angle $\theta$ is given by $\theta = \arctan(y/x)$, but we need to adjust for the correct quadrant. \textbf{2. Identify}...''}} \\[4pt]
\colorbox{orange!10}{\parbox{0.93\textwidth}{\textbf{Remaining tokens (execution):} ``...the Rectangular Coordinates: The given point is $(0,3)$. \textbf{3. Calculate $r$:} $r = \sqrt{0^2+3^2} = \sqrt{9} = 3$. \textbf{4. Determine $\theta$:} Since $x=0$ and $y=3$, the point lies on the positive y-axis. $\theta = \pi/2$.''}}
}}
\caption{\textbf{Strategy vs.\ execution in a student response.} The first $\sim$100 tokens (blue) establish the solution approach---which formulas to use, what to check. The remaining tokens (orange) execute the computation. Distillation on the strategy portion is sufficient because the student can already compute once it knows the approach.}
\label{fig:example}
\end{figure}
...
\midrule
Last 100 (tail) & 49.70\% & 58.40\% & 69.60\% & $-$1.3pp \\
Middle 100 (center) & 46.85\% & 56.60\% & 68.40\% & $-$4.1pp \\
\bottomrule
\end{tabular}
\end{table}
