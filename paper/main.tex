...
\colorbox{orange!10}{\parbox{0.93\textwidth}{\textbf{Tokens 101+ (execution):} ``...the Rectangular Coordinates: The given point is $(0,3)$. \textbf{3. Calculate $r$:} $r = \sqrt{0^2+3^2} = \sqrt{9} = 3$. \textbf{4. Determine $\theta$:} Since $x=0$ and $y=3$, the point lies on the positive y-axis. $\theta = \pi/2$.''}}
}}
\caption{\textbf{Strategy vs.\ execution in a student response.} The first $\sim$100 tokens (blue) establish the solution approach---which formulas to use, what to check. Tokens 101 onward (orange) execute the computation by substituting values and simplifying. Distillation on the strategy portion is sufficient because the student can already compute once it knows the approach.}
\label{fig:example}
\end{figure}
